...
- **WebSocket Proxy Support**: Automatic WebSocket upgrade handling with proper header forwarding. Each client connection gets one dedicated backend WebSocket leg for the lifetime of the session (nginx/Caddy style: no connection pooling, no sharing between clients). WebSocket sessions do not acquire or hold `locks:` cross-backend locks for the session lifetime (a parked tab must not block the rest of the domain), but they do take the full activation path: the backends the session's backend locks are deactivated before its own service starts, and open legs are closed automatically when a backend is deactivated.
...
- **WebSocket**: Automatically upgraded and proxied with proper headers. Each client session uses one dedicated backend WebSocket leg for its lifetime (nginx/Caddy style: no pooling, no sharing). WebSocket sessions do not acquire or hold `locks:` cross-backend locking for the session lifetime, but they keep the full activation path (activating a WebSocket backend deactivates the backends it locks), and legs are closed automatically when a backend is deactivated.
